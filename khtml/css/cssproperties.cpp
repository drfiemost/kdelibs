...
/* Command-line: gperf -a -L C++ -E -C -c -o -t -k '*' -Nfind -Hhash_prop -Wwordlist_prop -ZPropertyHash -D -s 2 cssproperties.gperf  */
...
{
private:
  static inline unsigned int hash_prop (const char *str, size_t len);
public:
  static const struct css_prop *find (const char *str, size_t len);
};

inline unsigned int
PropertyHash::hash_prop (const char *str, size_t len)
{
...
PropertyHash::find (const char *str, size_t len)
{
  enum
    {
      TOTAL_KEYWORDS = 179,
...
#line 146 "cssproperties.gperf"
  ...
  if (len <= MAX_WORD_LENGTH && len >= MIN_WORD_LENGTH)
    {
      unsigned int key = hash_prop (str, len);

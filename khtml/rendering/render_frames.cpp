    ...
    for (int k = 0; k < 2; ++k) {
        ...
        if (grid) {
            ...
            for (int i = 0; i < gridLen; ++i) {
                ...
                if (grid[i].isFixed()) {
                    gridLayout[i] = std::max(grid[i].value(), 0);
                ...
                if (grid[i].isPercent()) {
                    gridLayout[i] = std::max(grid[i].width(availableLen[k]), 0);
                ...
                if (grid[i].isRelative()) {
                    totalRelative += std::max(grid[i].value(), 1);
                    countRelative++;
                }
            }
            ...
            if (countRelative) {
                ...
                for (int i = 0; i < gridLen; ++i) {
                    if (grid[i].isRelative()) {
                        gridLayout[i] = (std::max(grid[i].value(), 1) * remainingRelative) / totalRelative;
                        remainingLen[k] -= gridLayout[i];
                        lastRelative = i;
                    }
                }

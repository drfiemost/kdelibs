...
void RenderBox::paintRootBoxDecorations(PaintInfo& paintInfo, int _tx, int _ty)
{
    ...
    int bw = std::max(w + marginLeft() + marginRight() + borderLeft() + borderRight(), rw);
    int bh = std::max(h + marginTop() + marginBottom() + borderTop() + borderBottom(), rh);
...
static void calculateBackgroundSize(const BackgroundLayer* bgLayer, int& scaledWidth, int& scaledHeight)
{
    ...
    if (bgLayer->isBackgroundSizeSet()) {
        if (bgLayer->backgroundSize().type == BGSLENGTH) {
            ...
            scaledWidth = std::max(1, w);
            scaledHeight = std::max(1, h);
        } else {
            ...
            float r = (bgLayer->backgroundSize().type == BGSCONTAIN) ? std::min(w,h) : std::max(w,h);
            scaledWidth = std::max(1, static_cast<int>(iw*r));
            scaledHeight = std::max(1, static_cast<int>(ih*r));
        }
    } else {
    ...
    if (shouldPaintBackgroundImage) {
        ...
        if (bgLayer->backgroundAttachment() != BGAFIXED) {
            ...
            if (bgr == NO_REPEAT || bgr == REPEAT_Y) {
                ...
                if ( xPosition >= 0 ) {
                    cx = _tx + xPosition;
                    cw = std::min(scaledImageWidth, pw - xPosition);
                }
                else {
                    cx = _tx;
                    if (scaledImageWidth > 0) {
                        sx = -xPosition;
                        cw = std::min(scaledImageWidth+xPosition, pw);
                    }
                }
                cx += left;
            } else {
            ...
            if (bgr == NO_REPEAT || bgr == REPEAT_X) {
                ...
                if ( yPosition >= 0 ) {
                    cy = _ty + yPosition;
                    ch = std::min(ch, ph - yPosition);
                }
                else {
                    cy = _ty;
                    if (scaledImageHeight > 0) {
                        sy = -yPosition;
                        ch = std::min(scaledImageHeight+yPosition, ph);
                    }
                }
        ...
        ch = std::min(ch, clipr.height());
        cw = std::min(cw, clipr.width());
...
QRect RenderBox::getFixedBackgroundImageRect( const BackgroundLayer* bgLayer, int& sx, int& sy, int& scaledImageWidth, int& scaledImageHeight )
{
    ...
    if (bgr == NO_REPEAT || bgr == REPEAT_X) {
        ch = std::min(scaledImageHeight, ph - yPosition);
        cy = vr.y() + yPosition;
    } else {
        ch = ph;
        cy = vr.y();
                                             ...
                                             int paddingLeft, int paddingRight, int paddingTop, int paddingBottom) const
{
    ...
    if (style()->hasBorderRadius()) {
        ...
        if (clipInner) {
            ...
            topLeftRadii.rx() = std::max(0, topLeftRadii.x() - adjustLeft);
            bottomLeftRadii.rx() = std::max(0, bottomLeftRadii.x() - adjustLeft);
            topRightRadii.rx() = std::max(0, topRightRadii.x() - adjustRight);
            bottomRightRadii.rx() = std::max(0, bottomRightRadii.x() - adjustRight);
            topLeftRadii.ry() = std::max(0, topLeftRadii.y() - adjustTop);
            topRightRadii.ry() = std::max(0, topRightRadii.y() - adjustTop);
            bottomLeftRadii.ry() = std::max(0, bottomLeftRadii.y() - adjustBottom);
            bottomRightRadii.ry() = std::max(0, bottomRightRadii.y() - adjustBottom);
        }
...
int RenderBox::calcWidthUsing(WidthType widthType, int cw, LengthType& lengthType)
{
    ...
    if (lengthType == Auto) {
        ...
        // size to max width?
        if (sizesToMaxWidth()) {
            width = std::max(width, (int)m_minWidth);
            width = std::min(width, (int)m_maxWidth);
        }
    }
    else
    {
...
void RenderBox::calcHeight()
{
    ...
    if (style()->overflowX() == OMARQUEE && m_layer && m_layer->marquee() &&
        ...
        m_height = std::min(m_height, m_layer->marquee()->unfurlPos());
    }
                                             ...
                                             short& widthValue, short& marginLeftValue, short& marginRightValue, short& xPos)
{
    ...
    if (!leftIsAuto && !widthIsAuto && !rightIsAuto) {
        ...
        if (leftIsAuto && widthIsAuto && !rightIsAuto) {
            ...
            widthValue = std::min(std::max(preferredMinWidth, availableWidth), preferredWidth);
                                           ...
                                           int& heightValue, short& marginTopValue, short& marginBottomValue, int& yPos)
{
    ...
    if (!topIsAuto && !heightIsAuto && !bottomIsAuto) {
        ...
        if (topIsAuto && heightIsAuto && !bottomIsAuto) {
            ...
            topValue = top.width(containerHeight);
            heightValue = std::max(0, availableSpace - (topValue + bottom.width(containerHeight)));
        } else if (!topIsAuto && !heightIsAuto && bottomIsAuto) {
...
//     kDebug(6040) << "_x " << _x << " _y " << _y;
	...
	if (!isReplaced() || style()->display() == BLOCK) {
	    if (!outsideEnd ^ rtl)
	        ...
	        _y += std::max(this->height() - fm.ascent() - hl, 0);
	} else {
